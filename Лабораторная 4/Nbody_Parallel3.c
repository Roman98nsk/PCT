...
void calculate_forces(struct particle *p, struct particle *f, float *m, int n)
{
  ...
  for (int i = 0; i < n-1; i++){
    for (int j = i + 1; j < n; j++){
      float dist = sqrtf(powf(p[i].x - p[j].x, 2) + powf(p[i].y - p[i].y, 2) + powf(p[i].z - p[j].z, 2));
...
int main(int argc, char *argv[], int stream)
{
  ...
  printf("# NBody (n=%d)\n", n);
  printf("# Elapsed time (sec): ttotal %.6f, tforces %.6f, tmove %.6f\n", ttotal, tinit, tforces, tmove);
